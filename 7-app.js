...
(function () {
  ...
  // Dev/demo hook: ?seed-demo creates a synthetic demo group and opens its admin view.
  if (location.search.indexOf('seed-demo') !== -1) {
    var dg = Store.createGroup('גן רימון — תשפ"ז (דוגמה)', 'הגננת דנה והצוות');
    Store.replaceSubmissions(dg.id, syntheticSubs());
    location.replace(location.pathname + '#/admin/' + dg.id);
  }
